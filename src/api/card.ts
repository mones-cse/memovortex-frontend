import type { TCreateCard, TReviewStudyCards, TUpdateCard } from "../types/card.type";
...
export const createCard = async (data: TCreateCard) => {
	...
	const response = await axiosInstance.post(`${API_URL}/v1/decks/${deckId}/cards`, rest);
...
export const updateCard = async (data: TUpdateCard) => {
	...
	const response = await axiosInstance.patch(`${API_URL}/v1/decks/${deckId}/cards/${id}`, rest);
	return response.data;
};

export const deleteCard = async ({ deckId, cardId }: { deckId: string; cardId: string }) => {
	const response = await axiosInstance.delete(`${API_URL}/v1/decks/${deckId}/cards/${cardId}`);
	return response;
};

export const fetchStudyCards = async (deckId: string) => {
	console.log("🚀 ~ fetchStudyCards ~ deckId:", deckId);
	const result = await axiosInstance.get(`${API_URL}/v1/decks/${deckId}/studies`);
	return result.data;
};
